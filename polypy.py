import sys, os, re, time, string, math, itertools, argparse, pdb
from copy import copy
## only ClayFF atoms
# element names
elements = ["O", "Al", "Si", "Ca" ]
## VDW radii
#radii = [1.52, 1.84, 2.1, 2.31 ]
## Lengthened radii to lengthen bonds
#radii = [2.0, 2.2, 2.2, 0.1 ]
### FV attempt to get all rings need to lengthen radii
radii = [2.12, 2.4, 2.4, 0.1 ]
...
def main(args):
  ...
  parser.add_argument("--n3r6", action="store_true", help="calculate number of \"SP2\"-atoms (three neighbors in a hexagon) (default=false)")
  ...
  if args.info or args.simple:
    ...
    if args.simple: # write header for simple nfo-file : columns for coordination, rings, chains
      ...
      if args.n3r6: nfo_file.write("   3n6r")
...
class graph:
  ...
  def print_info(g, f, args):
    ...
    if args.n3r6: 
        # get elements with only 3 neighbors which are in a ring 
        n3r6 = []
        for v in g.vertices:
          if v.nn == 3:
            #check if atom is in 6r
            for l in rl:
              if v.id in l and len(l) == 6:
                n3r6.append(v.id) 
        # print number of atoms with two neighbors and in six fold ring
        f.write('%s elements with 3 neighbors are in loop of size 6\n' % len(list(set(n3r6))))
        ...
        for v in g.vertices:
          if v.nn == 3:
            ...
            for l in rl:
              if v.id in l and len(l) == 6:
                n3r6.append(v.id) 
        # print number of atoms with three neighbors and in six fold ring
        f.write("%-4d" % len(list(set(n3r6)))+' ')
    if args.impropbool:
